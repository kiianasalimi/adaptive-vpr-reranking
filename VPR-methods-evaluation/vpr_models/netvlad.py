...
class NetVLAD(torch.nn.Module):
    def __init__(self, descriptors_dimension):
        ...
        if not checkpoint.exists():
            ...
            try:
                urllib.request.urlretrieve(link, str(checkpoint))
            except Exception as e:
                raise RuntimeError(
                    f"Failed to download NetVLAD checkpoint from {link} to {checkpoint}.\n"
                    f"Error: {e}\n"
                    f"Try downloading it manually in a browser and placing it at:\n{checkpoint}"
                )
